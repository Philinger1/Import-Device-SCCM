...
param (
[string]$Container="OSD",
[string]$DomainContainer="Domains",
[string]$Collection="All Users",

[string]$SiteCode="SITECODE",
[string]$SiteServer="SITESERVERNAME",
[string]$ApplicationVersion = "Import Device"
)

#Function to add the user names the combobox
function Set-UserNames {
   # $CollectionId = (Get-WmiObject -Class SMS_Collection -Namespace root/SMS/site_$($SiteCode) -ComputerName $SiteServer").CollectionId  -Filter "Name='$Collection'"
...
function Set-CollectionNames2 {
    $ContainerNodeId = (Get-WmiObject -Class SMS_ObjectContainerNode -Namespace root/SMS/site_$($SiteCode) -ComputerName $SiteServer -Filter "Name='$DomainContainer'").ContainerNodeId
    $Ids = Get-WmiObject -Class SMS_ObjectContainerItem -Namespace root/SMS/site_$($SiteCode) -ComputerName $SiteServer -Filter "ContainerNodeID='$ContainerNodeID'"
    foreach ($Id in $Ids) {
        $CollectionId = $Id.InstanceKey
        $CollectionName = (Get-WmiObject -Class SMS_Collection -Namespace root/SMS/site_$($SiteCode) -ComputerName $SiteServer -Filter "CollectionID='$CollectionId'").Name
        $comboBox3.Items.add($CollectionName)
    }
}
    ...
    $button1_OnClick= {
        Try {
            ...
            if($textBox1.Text.Length -eq 0 -or $maskedTextBox1.Text.Length -lt 17 -or $comboBox1.SelectedItem -eq $null -or ($comboBox2.SelectedItem -eq $null -and $checkbox1.Checked)) {
                ...
                $comboBox3.Text = "Select Prmary User:"
            }
        }
        Catch {
...
#Create comboBox2
    ...
    $linkLabel2.Text = "philipp.schindler@msg.group"
